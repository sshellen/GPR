            ...
            h = (function (e) {
              ...
              function f(e) {
                ...
                return (
                  ...
                  g(C(t), "viewEvents", function (n) {
                    "all" === n
                      ? (t.viewAllEvents.classList.add("selected"),
                        t.viewMonthlyEvents.classList.remove("selected"),
                        t.setState({ showMonth: !1 }, function () {
                          t.showAllEvents();
                        }))
                      : (t.viewMonthlyEvents.classList.add("selected"),
                        t.viewAllEvents.classList.remove("selected"),
                        t.setState({ showMonth: !0 }, function () {
                          (t.leftPaddle =
                            document.getElementById("leftPaddle")),
                            t.getDates();
                        }));
                  }),
                  g(C(t), "changeDate", function (n) {
                    "increment" === n
                      ? (t.setState({ month: t.state.month + 1 }, function () {
                          t.getDates(t.state.month);
                        }),
                        (t.leftPaddle.style.fill = "#011526"))
                      : t.state.curMonth <= t.state.month - 1 &&
                        t.setState({ month: t.state.month - 1 }, function () {
                          t.state.curMonth === t.state.month &&
                            (t.leftPaddle.style.fill = "#ccc"),
                            t.getDates(t.state.month);
                        });
                  }),
                  g(C(t), "showAllEvents", function () {
                    t.fetchData(
                      "".concat(n.env.API_URL, "/api/events?populate=*")
                    )
                      .then(function (n) {
                        return n.json();
                      })
                      .then(function (n) {
                        t.setState({ curMonth: s()().month() }),
                          t.setState({ events: n.data });
                      });
                  }),
                  g(C(t), "getDates", function () {
                    var e = s()()
                        .month(t.state.month)
                        .startOf("month")
                        .format("YYYY-MM-DD"),
                      ...
                      a = A.a.stringify({
                        filters: {
                          $and: [{ date: { $gte: e } }, { date: { $lte: o } }],
                        },
                      }),
                      l = ""
                        .concat(n.env.API_URL, "/api/events?")
                        .concat(a, "&populate=*");
                    t.fetchData(l)
                      .then(function (n) {
                        return n.json();
